  ...
  return (
    <Layout>
      <div style={{ padding: "20px" }}>
        <h1 style={{ fontSize: "28px", marginBottom: "30px" }}>Risk Monitor</h1>
        
        {riskData && (
          <div style={{ display: "flex", gap: "20px", marginBottom: "30px", flexWrap: "wrap" }}>
            <div style={cardStyle}>
              <h4 style={{ marginBottom: "8px", fontWeight: "600", color: "#666" }}>Total Routes</h4>
              <p style={{ fontSize: "22px", fontWeight: "bold", color: "#0d6efd", margin: 0 }}>{riskData.summary.total_routes}</p>
            </div>
            <div style={cardStyle}>
              <h4 style={{ marginBottom: "8px", fontWeight: "600", color: "#666" }}>High Risk Routes</h4>
              <p style={{ fontSize: "22px", fontWeight: "bold", color: "#dc3545", margin: 0 }}>{riskData.summary.high_risk_count}</p>
            </div>
            <div style={cardStyle}>
              <h4 style={{ marginBottom: "8px", fontWeight: "600", color: "#666" }}>Average Risk Score</h4>
              <p style={{ fontSize: "22px", fontWeight: "bold", color: "#0d6efd", margin: 0 }}>{riskData.summary.average_risk_score}</p>
            </div>
          </div>
        )}
        
        {!riskData?.data?.length ? (
          <p style={{ padding: "20px", color: "#666", textAlign: "center", background: "white", borderRadius: "8px" }}>
            No routes available yet. Create routes to see risk analysis.
          </p>
        ) : (
          <table style={{
            width: "100%",
